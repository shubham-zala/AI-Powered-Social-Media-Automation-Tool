...
import { Check, X, RefreshCw, LayoutTemplate, Edit2, Save, Sparkles, Trash2 } from 'lucide-react';
import api from '../services/api';
import RegenerateModal from '../components/RegenerateModal';

const ShortlistedQueue = () => {
    ...
    const fetchShortlisted = async () => {
        try {
            const res = await api.get('/posts?status=shortlisted');
            setPosts(res.data || []);
        } catch (error) {
            console.error(error);
            setPosts([]);
    ...
    const handleAction = async (id, status) => {
        try {
            ...
            if (status === 'approved') {
                setToastMessage('✅ Post moved to the Approved Queue!');
    ...
    if (posts.length === 0) {
        return (
            <div className="text-center py-20">
                <p className="text-slate-500 text-lg">No shortlisted posts to review.</p>
                <div className="mt-4">
                    <button onClick={fetchShortlisted} className="text-blue-400 hover:underline flex items-center justify-center gap-2 w-full">
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex justify-between items-end">
                <div>
                    <h2 className="text-2xl font-bold text-white">Shortlisted</h2>
                    <p className="text-slate-400">Review shortlisted posts and approve or reject them.</p>
                </div>
                <span className="text-neutral-500 text-sm">{posts.length} post{posts.length !== 1 ? 's' : ''} to review</span>
            </div>
                    ...
                    <div key={post.id} className="bg-black rounded-xl border border-neutral-800 overflow-hidden flex flex-col shadow-lg shadow-black/50 min-w-0">
                        ...
                        <div className="p-6 flex-1 flex flex-col">
                            ...
                            <div className="mb-3 flex items-center gap-2">
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 rounded-full text-xs font-medium">
                                    ⭐ Shortlisted
                                </span>
                            </div>
                            ...
                            <div className="mt-8 pt-6 border-t border-neutral-800 flex items-center gap-3">
                                ...
                                <button onClick={() => handleAction(post.id, 'approved')} className="flex-1 bg-green-600 hover:bg-green-500 text-white px-2 py-2 rounded-lg font-medium flex items-center justify-center gap-2 shadow-lg shadow-green-900/20">
...
export default ShortlistedQueue;
